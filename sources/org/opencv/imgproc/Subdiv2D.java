...
public class Subdiv2D {
    ...
    public Subdiv2D(Rect rect) {
        this.nativeObj = Subdiv2D_0(rect.f101x, rect.f102y, rect.width, rect.height);
    }
    ...
    public int edgeDst(int edge, Point dstpt) {
        ...
        if (dstpt != null) {
            dstpt.f96x = dstpt_out[0];
            dstpt.f97y = dstpt_out[1];
        }
        return retVal;
    }
    ...
    public int edgeOrg(int edge, Point orgpt) {
        ...
        if (orgpt != null) {
            orgpt.f96x = orgpt_out[0];
            orgpt.f97y = orgpt_out[1];
        }
        return retVal;
    }
    ...
    public int findNearest(Point pt, Point nearestPt) {
        ...
        int retVal = findNearest_0(this.nativeObj, pt.f96x, pt.f97y, nearestPt_out);
        if (nearestPt != null) {
            nearestPt.f96x = nearestPt_out[0];
            nearestPt.f97y = nearestPt_out[1];
        }
        return retVal;
    }

    public int findNearest(Point pt) {
        int retVal = findNearest_1(this.nativeObj, pt.f96x, pt.f97y);
        return retVal;
    }
    ...
    public int insert(Point pt) {
        int retVal = insert_0(this.nativeObj, pt.f96x, pt.f97y);
        return retVal;
    }
    ...
    public int locate(Point pt, int[] edge, int[] vertex) {
        ...
        int retVal = locate_0(this.nativeObj, pt.f96x, pt.f97y, edge_out, vertex_out);
        if (edge != null) {
    ...
    public void initDelaunay(Rect rect) {
        initDelaunay_0(this.nativeObj, rect.f101x, rect.f102y, rect.width, rect.height);
    }
