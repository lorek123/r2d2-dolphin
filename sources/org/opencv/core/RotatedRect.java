...
public class RotatedRect {
    ...
    public void points(Point[] pt) {
        ...
        pt[0] = new Point((this.center.f96x - (this.size.height * a)) - (this.size.width * b), (this.center.f97y + (this.size.height * b)) - (this.size.width * a));
        pt[1] = new Point((this.center.f96x + (this.size.height * a)) - (this.size.width * b), (this.center.f97y - (this.size.height * b)) - (this.size.width * a));
        pt[2] = new Point((2.0d * this.center.f96x) - pt[0].f96x, (2.0d * this.center.f97y) - pt[0].f97y);
        pt[3] = new Point((2.0d * this.center.f96x) - pt[1].f96x, (2.0d * this.center.f97y) - pt[1].f97y);
    }

    public Rect boundingRect() {
        Point[] pt = new Point[4];
        points(pt);
        Rect r = new Rect((int) Math.floor(Math.min(Math.min(Math.min(pt[0].f96x, pt[1].f96x), pt[2].f96x), pt[3].f96x)), (int) Math.floor(Math.min(Math.min(Math.min(pt[0].f97y, pt[1].f97y), pt[2].f97y), pt[3].f97y)), (int) Math.ceil(Math.max(Math.max(Math.max(pt[0].f96x, pt[1].f96x), pt[2].f96x), pt[3].f96x)), (int) Math.ceil(Math.max(Math.max(Math.max(pt[0].f97y, pt[1].f97y), pt[2].f97y), pt[3].f97y)));
        r.width -= r.f101x - 1;
        r.height -= r.f102y - 1;
        return r;
    }
    ...
    public int hashCode() {
        long temp = Double.doubleToLongBits(this.center.f96x);
        int result = ((int) ((temp >>> 32) ^ temp)) + 31;
        long temp2 = Double.doubleToLongBits(this.center.f97y);
