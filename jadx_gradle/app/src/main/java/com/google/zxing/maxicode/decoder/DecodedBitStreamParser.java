...
final class DecodedBitStreamParser {
        ...
        switch (mode) {
            ...
            case 3:
                ...
                if (result.toString().startsWith("[)>\u001e01\u001d")) {
                    result.insert(9, postcode + f62GS + country + f62GS + service + f62GS);
                    break;
                } else {
                    result.insert(0, postcode + f62GS + country + f62GS + service + f62GS);
                    break;
                }
            case 4:
    ...
    private static int getPostCode2(byte[] bytes) {
        return getInt(bytes, new byte[]{33, 34, 35, 36, 25, 26, 27, 28, 29, 30, 19, 20, 21, 22, 23, 24, Draft_75.f92CR, 14, 15, 16, 17, 18, 7, 8, 9, 10, 11, 12, 1, 2});
    }

    private static String getPostCode3(byte[] bytes) {
        return String.valueOf(new char[]{SETS[0].charAt(getInt(bytes, new byte[]{39, 40, 41, 42, 31, 32})), SETS[0].charAt(getInt(bytes, new byte[]{33, 34, 35, 36, 25, 26})), SETS[0].charAt(getInt(bytes, new byte[]{27, 28, 29, 30, 19, 20})), SETS[0].charAt(getInt(bytes, new byte[]{21, 22, 23, 24, Draft_75.f92CR, 14})), SETS[0].charAt(getInt(bytes, new byte[]{15, 16, 17, 18, 7, 8})), SETS[0].charAt(getInt(bytes, new byte[]{9, 10, 11, 12, 1, 2}))});
    }
